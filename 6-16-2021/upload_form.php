...
require_once($CFG->dirroot.'/phlcohort/phlcsv.php');
...
class cohort_upload_form extends moodleform {
    ...
    protected function process_upload_file($file, $encoding, $delimiter, $defaultcontext) {
        ...
        $allowedcolumns = array( 'name', 'idnumber', 'khoahoc','ngayhoc','ngaythi','khuvuc','quanhuyen','xaphuong','trainer','aduser','online','sonha');
        ...
        while ($row = $cir->next()) {
            ...
            if (!empty($hash['idnumber'])) {
                if (isset($idnumbers[$hash['idnumber']]) || $DB->record_exists('cohort', array('idnumber' => $hash['idnumber']))) {
                    $cohorts[$rownum]['warnings'][] = "Khoa hoc da ton tai";
            ...
            if (empty($hash['idnumber'])) {
                $cohorts[$rownum]['errors'][] ="ChÆ°a cÃ³ MÃ£ Lá»›p (TÃªn khÃ³a há» c)";
            }
            
            //Validate TinhThanh
            if (!empty($hash['khuvuc'])) {
                if (!$this->isexist_khuvuc($hash['khuvuc'])) {
                    $cohorts[$rownum]['errors'][] = "Tá»‰nh/ThÃ nh khÃ´ng tá»“n táº¡i trong Há»‡ thá»‘ng";
                }
                //$idnumbers[$hash['idnumber']] = true;
            }
            //Validate Course
            if (!empty($hash['khoahoc'])) {
                if (!$this->isexist_khoahoc($hash['khoahoc'])) {
                    $cohorts[$rownum]['errors'][] = "KhÃ³a há» c khÃ´ng tá»“n táº¡i trong Há»‡ thá»‘ng";
                }
                //$idnumbers[$hash['idnumber']] = true;
            }
        ...
        $render_cohort_exist = count($cohort_exist);
        $new_cohort = $rownum - $render_cohort_exist;
        echo "
<style>
table {
...
";
        echo "<table>
  <tr>
    <th>Update - Cap nhat</th>
    <th>Add - Them moi</th>
  </tr>
  <tr>
    <td>$render_cohort_exist</td>
    <td>$new_cohort</td>
  </tr>
</table>
";
