  ...
  fclose($myfile);
    ...
    if($khoahoc==0)
        ...
        if($khuvuc==0)
            ...
            if($mien>0 && $khuvuc>0 && $quanhuyen>0 && $khoahoc>0)
            {
                if (!isset($cohort->name)) {
                    throw new coding_exception('Missing cohort name in cohort_add_cohort().');
                }
                ...
                if($cohort->id>0)
                {
                    ...
                    $phlcohort = (object)array('cohortid' => $cohort->id,
                        'ngayhoc' => $intNgayhoc,'ngaythi' => $intNgaythi, 'khuvuc' => $quanhuyen, 'khoahoc' => $khoahoc,'thangnam'=>$yr.$mon,'sonha'=>$cohort->sonha,'trainer'=> $cohort->trainer,'aduser'=> $cohort->aduser,'online'=>$cohort->online);
